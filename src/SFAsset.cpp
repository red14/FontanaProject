...
SFAsset::SFAsset(SFASSETTYPE type) 
{
  ...
  {
  case SFASSET_PLAYER:
    tmp_surf = IMG_Load("assets/player2.png");
    break;
  case SFASSET_PROJECTILE:
    tmp_surf = IMG_Load("assets/projectile1.png");
    break;
  case SFASSET_ALIEN:
    tmp_surf = IMG_Load("assets/alien2.png");
    break;
  case SFASSET_COIN:
    tmp_surf = IMG_Load("assets/coin2.png");
    break;
  case SFASSET_BARRIER:
    tmp_surf = IMG_Load("assets/barrier.png");
   break;
  case SFASSET_BG:
    tmp_surf = IMG_Load("assets/background.png");
  break;
  }
...
void SFAsset::CGoSouth() 
  {
  Vector2 c = *(bbox->centre) + Vector2(0.0f, -6.5f);
  bbox->centre.reset();
  bbox->centre = make_shared<Vector2>(c);
  }
